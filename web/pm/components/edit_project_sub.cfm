...
<cfsilent>
    ...
    <cftry>
        ...
        <cfset eventText = prefiniti.getLongname(session.user.id) & " has edited a project.">
        <cfset prefiniti.writeUserEvent(session.user.id, "timeline_marker.png", eventText)>
    
        <cfset result.ok = true>
        <cfset result.message = "Project has been edited.">
        <cfset result.project_id = project.id>

        <cfcatch type="any">
            <cfset result.ok = false>
            <cfset result.message = "Error editing project.">  
